...
import {
  ...
  ChevronDown, ChevronUp, ChevronLeft, ChevronRight, MoveVertical, List, ListOrdered, Indent, Outdent, Type,
  SlidersHorizontal, ArrowRight, Star, Space
} from 'lucide-react';
...
export function EditorView(p: EditorViewProps) {
  ...
  // Letter Spacing (Espas) State
  const [localLetterSpacing, setLocalLetterSpacing] = useState<number>(0);
  const activeLetterSpacing = p.letterSpacing !== undefined ? p.letterSpacing : localLetterSpacing;
  const setLetterSpacing = (val: number) => {
    if (p.setLetterSpacing) {
      p.setLetterSpacing(val);
    } else {
  ...
  const transformText = (type: 'upper' | 'lower' | 'title' | 'trim', e?: React.MouseEvent) => {
    ...
    if (type !== 'trim') {
      setTransformFeedback(type);
      setTimeout(() => {
        setTransformFeedback(prev => (prev === type ? null : prev));
      }, 650);
    }
  ...
  // Termal Rulo Bölücü Çizgisi Ekle
  const insertDividerLine = (type: 'dashed' | 'double' | 'dotted' | 'solid', e?: React.MouseEvent) => {
    if (e) { e.preventDefault(); e.stopPropagation(); }
    const charMap = {
      dashed: '--------------------------------',
      double: '════════════════════════════════',
  ...
  // Özel Liste İmi Ekle (Ok veya Yıldız)
  const insertListPrefix = (prefix: 'arrow' | 'star', e?: React.MouseEvent) => {
    if (e) { e.preventDefault(); e.stopPropagation(); }
    const symbol = prefix === 'arrow' ? '→ ' : '★ ';
    const textarea = textareaRef.current;
    if (!textarea) {
    ...
    if (selected && selected.includes('\n')) {
      const lines = selected.split('\n');
      const newLines = lines.map(l => l.trim() ? `${symbol}${l.replace(/^([•→★]\s*|\d+\.\s*|\[\s*\]\s*)/, '')}` : l);
      insertTextAtCursor(newLines.join('\n'));
    } else {
      insertTextAtCursor(symbol);
    }
  ...
  // Hızlı Tipografi Önayarı Uygula
  const applyTypoPreset = (preset: 'headline' | 'subtitle' | 'receipt' | 'wide', e?: React.MouseEvent) => {
    if (e) { e.preventDefault(); e.stopPropagation(); }
    if (preset === 'headline') {
      p.setFontSize(32);
      p.setIsBold(true);
      ...
      p.setFontFamily('JetBrains Mono');
      p.setFontSize(16);
      p.setIsBold(false);
      p.setIsItalic(false);
      setLetterSpacing(0);
  ...
  // Seçili Metni Ters/Negatif veya Çerçeveye Sarma
  const wrapSelectedText = (type: 'invert' | 'box', e?: React.MouseEvent) => {
    if (e) { e.preventDefault(); e.stopPropagation(); }
    const textarea = textareaRef.current;
    if (!textarea) return;
    const start = textarea.selectionStart;
    const end = textarea.selectionEnd;
    const selected = p.text.substring(start, end) || 'ÖNEMLİ BAŞLIK';
    if (type === 'invert') {
      insertTextAtCursor(`■ ${selected.toUpperCase()} ■`);
    } else {
      const border = '═'.repeat(Math.max(16, selected.length + 4));
      insertTextAtCursor(`╔${border}╗\n║  ${selected}  ║\n╚${border}╝`);
    }
  };
  ...
  const handleTouchEnd = (e: React.TouchEvent, menu: 'font' | 'paragraph') => {
